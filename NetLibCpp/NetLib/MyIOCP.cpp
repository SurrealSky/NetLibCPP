...
MyIOCP* MyIOCP::Instantialize()
{
	if (pInstance == NULL)
	{   //double check
		CLock lock;           //用lock实现线程安全，用资源管理类，实现异常安全
...
{
	if (pContext->m_Socket == -1) return;//release版本否则会通知两次，第一次socket不为空，为主动DisconnectClient触发，第二次应该为IOWorkerThreadProc线程的GetQueuedCompletionStatus后触发，可能为bug
	unsigned int *pBuffer= new unsigned int;
	if(pBuffer)
		*pBuffer=pContext->m_Socket;
